...
import OrganizerTypeChoice from './OrganizerTypeChoice';
...
export default function RegisterPage({ onSwitchToLogin, onRegistered }: Props) {
  const { register, user, setUserRole } = useBackend();
  ...
  const handleTypeChoice = async (role: 'organizer' | 'observer') => {
